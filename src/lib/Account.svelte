...
{#if address}
  <div
    class="absolute top-4 right-4 flex flex-row justify-center items-center h-10 bg-base-200 rounded-xl px-1"
  >
    {#if balance != null}
      <div class=" text-lg font-medium px-4">{balance} {nativeSymbol}</div>
    {/if}
    <div
      class="flex justify-center items-center w-40 h-8 bg-base-500 rounded-xl text-xl text-white shadow-inner-input"
    >
      {address}
    </div>
  </div>
